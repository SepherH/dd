...
/**
 * 爬取圖片型態的監理所網頁
 * @param {string} url 監理所網頁 URL
 * @returns {Promise<Array>} 爬取到的資料陣列
 */
export async function imageCrawler(url) {
  try {
          ...
          try {
            ...
            if (extractedData && extractedData.length > 0) {
              extractedData.forEach(record => {
                ...
                record.crawlTime = new Date().toISOString();
                results.push(record);
              });
 ...
 * @param {CheerioElement} img 圖片元素
 * @returns {boolean} 是否可能包含目標資料
 */
function isPotentialOffenderImage($, img) {
  // 這個判斷邏輯需要根據實際網站結構調整
 ...
 * @returns {Promise<string|null>} 本地暫存路徑或 null
 */
